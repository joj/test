...
{
    public class ExepcSealPointAdapter : BaseAdapter<AndroidSealPoint>
    {
        ...
        public delegate void EventReceiveData(float fidppm, float fidmax, float pidppm, float pidmax);
        ...
        public ExepcSealPointAdapter(Activity contex, List<AndroidSealPoint> items, AndroidGroup group)
        {
            this.group = group;
            this.context = contex;
            this.items = items;
        ...
        public void Btn_Click(object sender, EventArgs e)
        {
            if (IsDataPolling == false)
            {
                ...
                {
                    Toast.MakeText(context, "请先连接WIFI", ToastLength.Short).Show();
                    return;
                }
            ...
            else
            {
                ...
                try
                {
                    ...
                    point.LeakagePPM = fidppm;
                    point.BackgroundPPM = backGroud.AvgValue;
                    point.PIDLeakagePPM = pidppm;
                    point.PIDBackgroundPPM = backGroud.PIDAvgValue;
                    ...
                    point.DetectionMode = UserPreferences.GetString("ExepcModel");
                    ...
                    if (point.PIDLeakagePPM >= 50000)
                    {
                        point.PIDPPM = 50000;
                    }
                    else
                    {
                        point.PIDPPM = point.PIDLeakagePPM - point.PIDBackgroundPPM;
                    }
                    if (point.PIDPPM < 0)
                    {
                        point.PIDPPM = 0;
                    }
                    point.PhxCode = App.exepcService.ExepcName;
        ...
        private void ExepcService_DataPoll(float FID, float PID)
        {
            ...
            if (context != null)
            {
                ...
                {
                    tvPPM.Text = fidppm.ToString("F0");
                    tvFlashPPM.Text = FID.ToString("F0");
                    DoReceiveData(FID, fidppm, PID, pidppm);
                });
            }
        }
