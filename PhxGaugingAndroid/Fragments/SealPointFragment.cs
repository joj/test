...
{
    public class SealPointFragment : Fragment
    {
        private AndroidGroup group;
        private List<AndroidGroup> groupList;
        private int position;
        public Bitmap btp;
        public SealPointFragment(List<AndroidGroup> groupList, int position)
        {
        ...
        {
            View v = inflater.Inflate(Resource.Layout.SealPoint, container, false);
        ...
        public override bool OnContextItemSelected(IMenuItem item)
        {
            ...
            try
            {
                ...
                var pointAdap = adapter as SealPointAdapter;
        ...
        private void SetSealPointListAdapter()
        {
            ...
            try
            {
                ...
                adapter = new SealPointAdapter(this.Activity, sealPointList, group);
                lvSealPoints.Adapter = adapter;
                adapter.SetComplateCount += Adapter_SetComplateCount;
                adapter.DoReceiveData += Adapter_DoReceiveData;
        ...
        {
            edtSealPointTime.Text = time.ToString();
        }
        private void Adapter_DoReceiveData(double ppm, double max)
        {
            tvSealPointPPM.Text = ppm.ToString();
            edtSealPointPPM.Text = max.ToString();
        }
    ...
    public class NoScrollListView : ListView
    {
        ...
        {
            if (IsScroll == false && e.Action == MotionEventActions.Move)
            {
                if (currentToast != null)
                {
                    currentToast.Cancel();
                }
                currentToast = Toast.MakeText(Context, "正在检测中请不要上下滑动", ToastLength.Short);
                currentToast.Show();
                return true;
            }
