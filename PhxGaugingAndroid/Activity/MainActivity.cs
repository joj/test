...
namespace PhxGaugingAndroid
{
        ...
        private void ListView_ItemClick(object sender, AdapterView.ItemClickEventArgs e)
        {
            ...
            if (this.androidBluetoothService.bluetoothDevice != null)
            {
                try
                {
                    ...
                    AndroidFileManager fileManager = new AndroidFileManager();
                    fileManager.DataDirectory = System.IO.Path.Combine(Android.OS.Environment.ExternalStorageDirectory.AbsolutePath, "LDARAPP6");
                    App.phxDeviceService = new PhxDeviceService(this.androidBluetoothService, device, fileManager);
                    App.phxDeviceService.Connect();
                    if (App.phxDeviceService.IsConnected)
                    {
                        //蓝牙
                        var phxDetail = F1FragmentList[F1Level] as PhxDetailFragment;
                        if (phxDetail != null)
                        {
                            phxDetail.ConnectBluetools(App.phxDeviceService);
                        }
                        dialog.Dismiss();
                        //标题显示连接信息
                        TextView tvPhxName = FindViewById<TextView>(Resource.Id.tvPhxName);
                        tvPhxName.Text = App.phxDeviceService.Name + "连接成功";
                        LinearLayout imgDiscover = FindViewById<LinearLayout>(Resource.Id.btnDiscover);
        ...
        void LoadFragment(int id)
        {
            ...
            switch (id)
            {
                case Resource.Id.tb1:
                    ...
                    {
                        PhxDetailFragment f1 = F1FragmentList[F1Level] as PhxDetailFragment;
                        if (f1 == null)
                        {
                            f1 = PhxDetailFragment.NewInstance();
                            f1.GotoCalibration += Detail_GotoCalibration;
                            f1.GotoDiagnostic += Detail_GotoDiagnostic;
                        }
                        if (App.phxDeviceService != null && f1.phxDeviceService == null)
                        {
                            f1.ConnectBluetools(App.phxDeviceService);
                        }
                        fragment = f1;
                        F1Level = 0;
                    }
                ...
                case Resource.Id.tb3:
                    ...
                    {
                        GaugingFragment f3 = F3FragmentList[F3Level] as GaugingFragment;
                        if (f3 == null)
                        {
                            f3 = GaugingFragment.NewInstance();
                        }
                        fragment = f3;
                        F3Level = 0;
                    }
    ...
    public interface StopReceiveData
    {
